...
/*   Updated: 2022/01/07 20:12:46 by alorain          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	convert_buff(char *str, int idx)
{
	int	i;
	int	idx_str;
	...
	while (i < idx)
	{
		if (i && i % 8 == 0)
		{
			str[i] = ft_bitoa(&g_talk.buffer[i - 8]);
			ft_printf("char =%c\n", ft_bitoa(&g_talk.buffer[i - 8]));
			/* int index;

			index =  0;
			while (index <= idx)
			{ */
				ft_printf("str =%c\n",g_talk.buffer[/* index */0]);
			/* 	index++;
			} */
			idx_str++;
		}
...
void	manage_buffer(int idx)
{
	char	*str;

	str = malloc(sizeof(char) * (idx / 8));
	convert_buff(str, idx);
	ft_printf("sizeof str = %d\n", idx / 8);
	ft_printf("buffer str = %s\n", str );
	g_talk.str = str;
}
